                                              ...
                                              std::shared_ptr<AuthenticateCallback> callback)
{
    ...
    std::shared_ptr<IpcRsp> rsp = std::make_shared<IpcRsp>();
    req->SetPkgName(pkgName);
    req->SetExtra(extraJson.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore));
...
{
    if (pkgName.empty()) {
        DmRadarHelper::GetInstance().ReportDmBehavior(pkgName, "StopAuthenticateDevice", ERR_DM_INPUT_PARA_INVALID);
    ...
    if (ret != DM_OK) {
        DmRadarHelper::GetInstance().ReportDmBehavior(pkgName, "StopAuthenticateDevice", ret);
    ...
    const std::string &bindParam, std::shared_ptr<AuthenticateCallback> callback)
{
    ...
    std::shared_ptr<IpcRsp> rsp = std::make_shared<IpcRsp>();
    req->SetPkgName(pkgName);
    req->SetBindParam(paramJson.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore));
