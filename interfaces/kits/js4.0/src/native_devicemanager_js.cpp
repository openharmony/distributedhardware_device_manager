...
napi_value DeviceManagerNapi::CreateDeviceManager(napi_env env, napi_callback_info info)
{
    ...
    if (ret != 0) {
        LOGE("CreateDeviceManager for bundleName %s failed, ret %d.", bundleName.c_str(), ret);
        CreateBusinessError(env, ret);
        return nullptr;
    }
    {
    ...
    if (status != napi_ok) {
        LOGE("Create DeviceManagerNapi for bundleName %s failed", bundleName.c_str());
    }
    return result;
}
...
napi_value DeviceManagerNapi::Constructor(napi_env env, napi_callback_info info)
{
    ...
    LOGI("create DeviceManagerNapi for packageName:%s", bundleName.c_str());
