...
void SoftbusAdapter::OnDeviceStateChange(DmDeviceState state, DmDeviceInfo &deviceInfo)
{
    std::list<std::string> pkgNameList = DeviceManagerNotify::GetInstance().GetPkgNameList();
...
#else
    OnDeviceStateChange(DmDeviceState::DEVICE_STATE_ONLINE, dmDeviceInfo);
#endif
...
#else
    OnDeviceStateChange(DmDeviceState::DEVICE_STATE_OFFLINE, dmDeviceInfo);
#endif
}
...
#else
            DeviceManagerNotify::GetInstance().OnDeviceFound(strPkgName, originId, dmDeviceInfo);
#endif
        }
    }
}
...
#else
    DeviceManagerNotify::GetInstance().OnDiscoverFailed(pkgName, originId, failReason);
#endif
}
...
#else
    DeviceManagerNotify::GetInstance().OnDiscoverySuccess(pkgName, originId);
#endif
}
...
void PublishServiceCallBack::OnPublishSuccess(int publishId)
{
    DMLOG(DM_LOG_INFO, "service publish succeed, publishId: %d", publishId);
}

void PublishServiceCallBack::OnPublishFail(int publishId, PublishFailReason reason)
{
