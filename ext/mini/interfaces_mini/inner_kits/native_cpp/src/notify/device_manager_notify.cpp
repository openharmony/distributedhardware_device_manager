...
void DeviceManagerNotify::AddPkgName(std::string &pkgName)
{
    dmPkgName_.push_back(pkgName);
}

void DeviceManagerNotify::DeletePkgName(std::string &pkgName)
{
    dmPkgName_.remove(pkgName);
}

const std::list<std::string>& DeviceManagerNotify::GetPkgNameList() const
{
    return dmPkgName_;
}
