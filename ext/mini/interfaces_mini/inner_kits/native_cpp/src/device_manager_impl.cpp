...
int32_t DeviceManagerImpl::InitDeviceManager(std::string &pkgName, std::shared_ptr<DmInitCallback> dmInitCallback)
{
    ...
    DeviceManagerNotify::GetInstance().AddPkgName(pkgName);
...
int32_t DeviceManagerImpl::UnInitDeviceManager(std::string &pkgName)
{
    ...
    DeviceManagerNotify::GetInstance().DeletePkgName(pkgName);
    ...
    std::vector<DmDeviceInfo> &deviceList)
{
    ...
    if (CommandDispatch::GetInstance().MessageSendCmd(GET_TRUST_DEVICE_LIST,  req, rsp) != DEVICEMANAGER_OK) {
    ...
    std::shared_ptr<DiscoverCallback> callback)
{
    ...
    if (CommandDispatch::GetInstance().MessageSendCmd(START_DEVICE_DISCOVER,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
...
int32_t DeviceManagerImpl::StopDeviceDiscovery(std::string &pkgName, uint16_t subscribeId)
{
    ...
    if (CommandDispatch::GetInstance().MessageSendCmd(STOP_DEVICE_DISCOVER,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
    int32_t ret = rsp->GetErrCode();
    if (ret != DEVICEMANAGER_OK) {
...
int32_t DeviceManagerImpl::SetUserOperation(std::string &pkgName, int32_t action)
{
    ...
    std::shared_ptr<GetOperationReq> req = std::make_shared<GetOperationReq>();
    std::shared_ptr<MessageRsp> rsp = std::make_shared<MessageRsp>();

    req->SetPkgName(pkgName);
    req->SetOperation(action);

    if (CommandDispatch::GetInstance().MessageSendCmd(SERVER_USER_AUTHORIZATION_OPERATION,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
    int32_t ret = rsp->GetErrCode();
    if (ret != DEVICEMANAGER_OK) {
...
int32_t DeviceManagerImpl::GetAuthenticationParam(std::string &pkgName, DmAuthParam &authParam)
{
    ...
    std::shared_ptr<GetAuthParamRsp> rsp = std::make_shared<GetAuthParamRsp>();
    req->SetPkgName(pkgName);
    if (CommandDispatch::GetInstance().MessageSendCmd(SERVER_GET_AUTHENTCATION_INFO,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
    ...
    const DmAppImageInfo &imageInfo, std::string &extra, std::shared_ptr<AuthenticateCallback> callback)
{
    ...
    if (CommandDispatch::GetInstance().MessageSendCmd(AUTHENTICATE_DEVICE,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
    ...
    std::shared_ptr<CheckAuthCallback> callback)
{
    ...
    std::shared_ptr<CheckAuthenticateReq> req = std::make_shared<CheckAuthenticateReq>();
    std::shared_ptr<MessageRsp> rsp = std::make_shared<MessageRsp>();
    req->SetPkgName(pkgName);
    req->SetAuthPara(authPara);

    if (CommandDispatch::GetInstance().MessageSendCmd(CHECK_AUTHENTICATION,  req, rsp) != DEVICEMANAGER_OK) {
       return DEVICEMANAGER_MESSAGE_FAILED;
    }
