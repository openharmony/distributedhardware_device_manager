...
DistributedHardwareLoadCallback::DistributedHardwareLoadCallback(
    const uint32_t &nLoadCount) : nLoadCount_(nLoadCount) {}
...
void DistributedHardwareLoadCallback::OnLoadSystemAbilityFail(int32_t systemAbilityId)
{
    ...
    uint32_t minLoadValue = 1;
    if (nLoadCount_ > minLoadValue) {
        DmLoadFwk::GetInstance().LoadFwk();
        nLoadCount_--;
    }
    return;
}
