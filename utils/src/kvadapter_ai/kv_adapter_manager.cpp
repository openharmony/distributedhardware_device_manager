...
DM_IMPLEMENT_SINGLE_INSTANCE(KVAdapterManagerAI);
...
DM_EXPORT int32_t KVAdapterManagerAI::Init()
{
...
DM_EXPORT void KVAdapterManagerAI::UnInit()
{
...
DM_EXPORT void KVAdapterManagerAI::ReInit()
{
...
int32_t KVAdapterManagerAI::PutByAnoyDeviceId(const std::string &key, const DmKVValue &value)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::Get(const std::string &key, DmKVValue &value)
{
    std::string dmKey = DM_KV_STORE_PREFIX + key;
    {
        std::lock_guard<ffrt::mutex> lock(idCacheMapMtx_);
...
DM_EXPORT int32_t KVAdapterManagerAI::DeleteAgedEntry()
{
...
inline bool KVAdapterManagerAI::IsTimeOut(int64_t sourceTime, int64_t targetTime, int64_t timeOut)
{
    return targetTime - sourceTime >= timeOut ? true : false;
}

DM_EXPORT int32_t KVAdapterManagerAI::AppUninstall(const std::string &appId)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::GetFreezeData(const std::string &key, std::string &value)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::PutFreezeData(const std::string &key, std::string &value)
{
    std::string dmKey = DM_KV_STORE_FREEZE_PREFIX + key;
    std::lock_guard<ffrt::mutex> kvAdapterLck(kvAdapterMtx_);
    CHECK_NULL_RETURN(kvAdapter_, ERR_DM_POINT_NULL);
    if (kvAdapter_->Put(dmKey, value) != DM_OK) {
        LOGE("Insert freeze data failed, k:%{public}s", GetAnonyString(dmKey).c_str());
        return ERR_DM_FAILED;
    }
    LOGI("Insert freeze data success, k:%{public}s", GetAnonyString(dmKey).c_str());
    return DM_OK;
}

DM_EXPORT int32_t KVAdapterManagerAI::DeleteFreezeData(const std::string &key)
{
    std::string dmKey = DM_KV_STORE_FREEZE_PREFIX + key;
    std::lock_guard<ffrt::mutex> kvAdapterLck(kvAdapterMtx_);
    CHECK_NULL_RETURN(kvAdapter_, ERR_DM_POINT_NULL);
...
DM_EXPORT int32_t KVAdapterManagerAI::GetAllOstypeData(std::vector<std::string> &values)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::PutOstypeData(const std::string &key, const std::string &value)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::DeleteOstypeData(const std::string &key)
{
...
DM_EXPORT int32_t KVAdapterManagerAI::GetLocalUserIdData(const std::string &key, std::string &value)
{
    LOGI("key %{public}s.", GetAnonyString(key).c_str());
    std::lock_guard<ffrt::mutex> kvAdapterLck(kvAdapterMtx_);
    CHECK_NULL_RETURN(kvAdapter_, ERR_DM_POINT_NULL);
...
DM_EXPORT int32_t KVAdapterManagerAI::PutLocalUserIdData(const std::string &key, const std::string &value)
{
    LOGI("key %{public}s, value %{public}s.", GetAnonyString(key).c_str(), value.c_str());
    std::lock_guard<ffrt::mutex> kvAdapterLck(kvAdapterMtx_);
    CHECK_NULL_RETURN(kvAdapter_, ERR_DM_POINT_NULL);
...
DM_EXPORT int32_t KVAdapterManagerAI::GetOsTypeCount(int32_t &count)
{
    std::lock_guard<ffrt::mutex> kvAdapterLck(kvAdapterMtx_);
    CHECK_NULL_RETURN(kvAdapter_, ERR_DM_POINT_NULL);
