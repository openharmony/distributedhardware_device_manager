...
int32_t HiChainAuthConnector::AuthDevice(int32_t pinCode, int32_t osAccountId, std::string udid, int64_t requestId)
{
    ...
    std::string authParam = authParamJson.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore);
...
int32_t HiChainAuthConnector::ProcessAuthData(int64_t requestId, std::string authData, int32_t osAccountId)
{
    ...
    int32_t ret = ProcessAuthDevice(requestId,
        jsonAuthParam.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore).c_str(), &deviceAuthCallback_);
    if (ret != HC_SUCCESS) {
...
int32_t HiChainAuthConnector::GenerateCredential(std::string &localUdid, int32_t osAccountId, std::string &publicKey)
{
    ...
    jsonObj["flag"] = 1;
    std::string requestParam = jsonObj.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore);
