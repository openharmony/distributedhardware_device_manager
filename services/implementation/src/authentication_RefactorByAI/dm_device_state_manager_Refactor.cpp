...
constexpr uint32_t DM_EVENT_QUEUE_CAPACITY = 20;
constexpr uint32_t DM_EVENT_WAIT_TIMEOUT = 2;
...
constexpr uint32_t AUTH_ONCE_STATE_TIMER_MAX = 500;

NotifyEvent::NotifyEvent(int32_t eventId, const std::string &deviceId)
    : eventId_(eventId), deviceId_(deviceId)
{
}
...
int32_t NotifyEvent::GetEventId() const
{
    return eventId_;
}

std::string NotifyEvent::GetDeviceId() const
{
    return deviceId_;
}
...
bool DmDeviceStateManager::IsValidDeviceId(const std::string &deviceId) const
{
    if (deviceId.empty()) {
        return false;
    }
    if (deviceId.length() > DM_MAX_DEVICE_ID_LEN) {
        return false;
    }
    return true;
}

bool DmDeviceStateManager::IsValidStateTransition(DmDeviceState currentState, DmDeviceState newState) const
{
    switch (currentState) {
        case DEVICE_STATE_OFFLINE:
            return (newState == DEVICE_STATE_ONLINE || newState == DEVICE_INFO_CHANGED);
        case DEVICE_STATE_ONLINE:
            return (newState == DEVICE_STATE_OFFLINE || newState == DEVICE_INFO_CHANGED);
        case DEVICE_INFO_READY:
            return (newState == DEVICE_STATE_ONLINE || newState == DEVICE_STATE_OFFLINE ||
                    newState == DEVICE_INFO_CHANGED);
        case DEVICE_INFO_CHANGED:
            return (newState == DEVICE_STATE_ONLINE || newState == DEVICE_STATE_OFFLINE);
        default:
            return (newState == DEVICE_STATE_ONLINE || newState == DEVICE_STATE_OFFLINE ||
                    newState == DEVICE_INFO_CHANGED || newState == DEVICE_INFO_READY);
    }
}

void DmDeviceStateManager::ClearSensitiveData(std::string &data) const
{
    if (!data.empty()) {
        (void)memset_s(&data[0], data.size(), 0, data.size());
        data.clear();
    }
}
...
void DmDeviceStateManager::SaveOnlineDeviceInfo(const DmDeviceInfo &info)
{
    ...
    if (!IsValidDeviceId(std::string(info.deviceId))) {
...
void DmDeviceStateManager::DeleteOfflineDeviceInfo(const DmDeviceInfo &info)
{
    ...
    std::string targetDeviceId(info.deviceId);
    {
#if !(defined(__LITEOS_M__) || defined(LITE_DEVICE))
        std::lock_guard<ffrt::mutex> mutexLock(remoteDeviceInfosMutex_);
#else
        std::lock_guard<std::mutex> mutexLock(remoteDeviceInfosMutex_);
#endif
        for (auto iter = remoteDeviceInfos_.begin(); iter != remoteDeviceInfos_.end(); ++iter) {
            if (std::string(iter->second.deviceId) == targetDeviceId) {
                remoteDeviceInfos_.erase(iter);
                LOGI("Delete remoteDeviceInfos complete");
                break;
            }
        }
        for (auto iter = stateDeviceInfos_.begin(); iter != stateDeviceInfos_.end(); ++iter) {
            if (std::string(iter->second.deviceId) == targetDeviceId) {
                stateDeviceInfos_.erase(iter);
    ...
    }
    {
        ...
        for (auto iter = notifyEventInfos_.begin(); iter != notifyEventInfos_.end(); ++iter) {
            if (std::string(iter->second.deviceId) == targetDeviceId) {
                notifyEventInfos_.erase(iter);
...
void DmDeviceStateManager::OnDeviceOffline(std::string deviceId, const bool isOnline)
{
    ...
    DmDeviceInfo devInfo;
    {
        ...
        std::lock_guard<std::mutex> mutexLock(remoteDeviceInfosMutex_);
#endif
        auto iter = stateDeviceInfos_.find(deviceId);
...
void DmDeviceStateManager::HandleOnline(DmDeviceState devState, DmDeviceInfo &devInfo, const bool isOnline)
{
    ...
    std::string requestDeviceId(localUdid);
    std::vector<int32_t> foregroundUserIds;
    MultipleUserConnector::GetForegroundUserIds(foregroundUserIds);
...
void DmDeviceStateManager::RegisterOffLineTimer(const DmDeviceInfo &deviceInfo)
{
    ...
    for (const auto &peerUserId : peerUserIds) {
        ...
        if (stateTimerInfoMap_.size() > STATE_TIMER_MAP_MAX_SIZE) {
        ...
        stateTimer.timerName = timerName;
        stateTimer.peerUdid = peerUdid;
        stateTimer.peerUserId = peerUserId;
        stateTimer.localUserId = localUserId;
        stateTimer.isStart = false;
...
void DmDeviceStateManager::ThreadLoop()
{
    ...
    while (running) {
        ...
        {
            std::unique_lock<std::mutex> lock(eventTask_.queueMtx_);
            while (eventTask_.queue_.empty() && running) {
                eventTask_.queueCond_.wait_for(lock, std::chrono::seconds(DM_EVENT_WAIT_TIMEOUT));
                {
                    std::lock_guard<std::mutex> runLock(threadRunningMutex_);
                    running = eventTask_.threadRunning_;
                }
            }
            if (!eventTask_.queue_.empty() && running) {
...
std::string DmDeviceStateManager::GetUdidByNetWorkId(const std::string &networkId)
{
    ...
    }
    {
        ...
        std::lock_guard<std::mutex> mutexLock(remoteDeviceInfosMutex_);
#endif
        for (const auto &iter : stateDeviceInfos_) {
...
int32_t DmDeviceStateManager::DeleteGroupByDP(const std::string &peerUdid, int32_t peerUserId, int32_t localUserId)
{
    ...
    for (const auto &iter : groupListExt) {
        for (const auto &pkgName : delPkgNameVec) {
