...
int32_t DmDeviceFilterOption::TransformToFilter(const std::string &filterOptions)
{
    if (filterOptions.empty()) {
        ...
        deviceFilters.value = DmDiscoveryDeviceFilter::DM_OFFLINE_DEVICE;
...
{
    if (value == DmDiscoveryDeviceFilter::DM_OFFLINE_DEVICE) {
        return !isOnline;
    }
    if (value == DmDiscoveryDeviceFilter::DM_ONLINE_DEVICE) {
        return isOnline;
    }
    return (value == DmDiscoveryDeviceFilter::DM_ALL_DEVICE);
}
