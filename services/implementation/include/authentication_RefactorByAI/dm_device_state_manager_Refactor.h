...
constexpr int32_t OFFLINE_TIMEOUT = 300;
constexpr size_t STATE_TIMER_MAP_MAX_SIZE = 500;
...
class NotifyEvent {
public:
    NotifyEvent(int32_t eventId, const std::string &deviceId);
    ~NotifyEvent();
...
public:
    ...
    struct OfflineHandleParam {
        ...
        DmDeviceState devState = DEVICE_STATE_OFFLINE;
        bool isOnline = false;
    };
...
private:
    ...
    bool IsValidStateTransition(DmDeviceState currentState, DmDeviceState newState) const;
    void ClearSensitiveData(std::string &data) const;
#if !(defined(__LITEOS_M__) || defined(LITE_DEVICE))
