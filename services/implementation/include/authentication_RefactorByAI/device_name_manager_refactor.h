...
#ifndef OHOS_DEVICE_NAME_MANAGER_REFACTOR_H
#define OHOS_DEVICE_NAME_MANAGER_REFACTOR_H
...
class DeviceNameManagerRefactor {
    DM_DECLARE_SINGLE_INSTANCE_BASE(DeviceNameManagerRefactor);
public:
    int32_t UnInit();
...
private:
    DeviceNameManagerRefactor() = default;
    ~DeviceNameManagerRefactor() = default;
    bool IsDependsReady() const;
    bool EnsureAccountSysReady(int32_t userId);
    bool ValidateUserId(int32_t userId) const;
    ...
    std::string TruncateSuffixToLength(const std::string &text, int32_t maxLen);
...
private:
    ...
    bool localMarketNameInitialized_ = false;
...
#endif // OHOS_DEVICE_NAME_MANAGER_REFACTOR_H
