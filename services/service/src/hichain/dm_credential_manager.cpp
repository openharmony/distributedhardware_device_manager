...
DmCredentialManager::DmCredentialManager(std::shared_ptr<DmServiceHiChainConnector> hiChainConnector,
...
int32_t DmCredentialManager::RegisterCredentialCallback(const std::string &pkgName)
{
    ...
    return hiChainConnector_->RegisterHiChainGroupCallback(
        std::shared_ptr<IDmServiceGroupResCallback>(shared_from_this()));
}
...
void FromJson(const JsonItemObject &jsonObject, CredentialDataInfo &credentialDataInfo)
{
    ...
    if (IsString(jsonObject, FIELD_CREDENTIAL_ID)) {
        credentialDataInfo.credentialId = jsonObject[FIELD_CREDENTIAL_ID].Get<std::string>();
    }
...
void ToJson(JsonItemObject &jsonObject, const CredentialDataInfo &credentialDataInfo)
{
    ...
    jsonObject[FIELD_CREDENTIAL_ID] = atoi(credentialDataInfo.credentialId.c_str());
    if (credentialDataInfo.credentialType == NONSYMMETRY_CREDENTIAL_TYPE) {
