...
#include "pin_holder_session_Refactor.h"
...
constexpr int32_t MAX_DATA_LEN = 4096000;
...
int32_t PinHolderSession::OpenSessionServer(const PeerTargetId &targetId)
{
    ...
    if (!IsAddrTypeValid(addrInfo)) {
        LOGE("[SOFTBUS]addr type invalid after GetAddrByTargetId.");
...
int32_t PinHolderSession::GetAddrByTargetId(const PeerTargetId &targetId, ConnectionAddr &addr)
{
    if (!targetId.wifiIp.empty() && targetId.wifiIp.length() < IP_STR_MAX_LEN) {
        ...
        LOGE("no valid address type in targetId.");
        return ERR_DM_INPUT_PARA_INVALID;
    }
    return DM_OK;
}

bool PinHolderSession::IsAddrTypeValid(const ConnectionAddr &addrInfo)
{
