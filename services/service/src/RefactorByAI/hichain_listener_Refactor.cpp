...
#include "hichain_listener_Refactor.h"
...
namespace {
    ...
    constexpr size_t MIN_UDID_LEN = 1;
}
...
bool HichainListener::ValidateUserId(int32_t userId)
{
    if (userId < 0) {
...
bool HichainListener::ValidatePeerUdid(const char *peerUdid)
{
    if (peerUdid == nullptr) {
        LOGE("peerUdid is null.");
        return false;
    }
    size_t len = strlen(peerUdid);
    if (len < MIN_UDID_LEN || len > MAX_DATA_LEN) {
        LOGE("peerUdid length is invalid: %{public}zu.", len);
        return false;
    }
    for (size_t i = 0; i < len; i++) {
        char ch = peerUdid[i];
        if (!std::isalnum(static_cast<unsigned char>(ch)) && ch != '_' && ch != '-' && ch != '.') {
            LOGE("peerUdid contains invalid character at position %{public}zu.", i);
            return false;
        }
    }
    return true;
...
void HichainListener::OnHichainDeviceUnBound(const char *peerUdid, const char *groupInfo)
{
    ...
    if (!ValidatePeerUdid(peerUdid)) {
        LOGE("peerUdid format validation failed.");
        return;
    }
...
void HichainListener::DeleteAllGroup(const std::string &localUdid, const std::vector<int32_t> &backgroundUserIds)
{
    ...
    for (auto &userId : backgroundUserIds) {
        if (!ValidateUserId(userId)) {
            LOGE("Skipping invalid userId in backgroundUserIds.");
            continue;
        }
...
int32_t HichainListener::DeleteGroup(const int32_t userId, std::string &groupId)
{
    ...
    ClearSensitiveString(disbandParams);
    if (ret != 0) {
