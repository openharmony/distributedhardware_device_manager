...
HWTEST_F(AuthAclTest, GetAppTrustDeviceList_NonSA_TokenIdZero_001, testing::ext::TestSize.Level1)
{
    ...
    auto appManagerMock = std::make_shared<AppManagerMock>();
    DmAppManager::dmAppManager = appManagerMock;
    ...
    profile.SetTrustDeviceId("peerDeviceId");
    ...
    EXPECT_CALL(*appManagerMock, IsSystemSA()).WillOnce(Return(false));
    EXPECT_CALL(*skeletonMock, GetCallingTokenID()).WillOnce(Return(0));
    EXPECT_CALL(*deviceProfileConnectorMock, GetAllAclIncludeLnnAcl()).WillOnce(Return(profiles));
    EXPECT_CALL(*multipleUserConnectorMock, GetCurrentAccountUserID()).WillOnce(Return(100));

    auto result = DeviceProfileConnector::GetInstance().GetAppTrustDeviceList(pkgName, deviceId);
...
HWTEST_F(AuthAclTest, GetAppTrustDeviceList_NonSA_MatchAsAccesser_001, testing::ext::TestSize.Level1)
{
    auto skeleton = IPCSkeletonInterface::GetOrCreateIPCSkeleton();
    auto skeletonMock = std::static_pointer_cast<IPCSkeletonMock>(skeleton);
    auto appManagerMock = std::make_shared<AppManagerMock>();
    ...
    accessee.SetAccesseeDeviceId(peerDeviceId);
    accessee.SetAccesseeBundleName(pkgName);
    accessee.SetAccesseeTokenId(54321);
    ...
    EXPECT_CALL(*deviceProfileConnectorMock, GetAllAclIncludeLnnAcl()).WillOnce(Return(profiles));
    EXPECT_CALL(*deviceProfileConnectorMock, IsLnnAcl(_)).WillOnce(Return(false));
    EXPECT_CALL(*multipleUserConnectorMock, GetCurrentAccountUserID()).WillOnce(Return(100));
    EXPECT_CALL(*deviceProfileConnectorMock, CheckSinkShareType(_, _, _, _, _)).WillOnce(Return(false));

    auto result = DeviceProfileConnector::GetInstance().GetAppTrustDeviceList(pkgName, deviceId);
    EXPECT_FALSE(result.empty());
    EXPECT_EQ(result.find(peerDeviceId)->second, DmAuthForm::PEER_TO_PEER);

    IPCSkeletonInterface::ReleaseIPCSkeleton();
    DmAppManager::dmAppManager = nullptr;
}

HWTEST_F(AuthAclTest, GetAppTrustDeviceList_NonSA_MatchAsAccessee_001, testing::ext::TestSize.Level1)
{
    auto skeleton = IPCSkeletonInterface::GetOrCreateIPCSkeleton();
    auto skeletonMock = std::static_pointer_cast<IPCSkeletonMock>(skeleton);
    auto appManagerMock = std::make_shared<AppManagerMock>();
    ...
    DistributedDeviceProfile::Accesser accesser;
    accesser.SetAccesserUserId(100);
    accesser.SetAccesserDeviceId(peerDeviceId);
    accesser.SetAccesserBundleName(pkgName);
    accesser.SetAccesserTokenId(54321);
    profile.SetAccesser(accesser);
...
HWTEST_F(AuthAclTest, GetAppTrustDeviceList_NonSA_TokenIdNotMatch_001, testing::ext::TestSize.Level1)
{
    auto skeleton = IPCSkeletonInterface::GetOrCreateIPCSkeleton();
    auto skeletonMock = std::static_pointer_cast<IPCSkeletonMock>(skeleton);
    auto appManagerMock = std::make_shared<AppManagerMock>();
...
HWTEST_F(AuthAclTest, GetAppTrustDeviceList_SystemSA_BundleNameMatch_001, testing::ext::TestSize.Level1)
{
    auto skeleton = IPCSkeletonInterface::GetOrCreateIPCSkeleton();
    auto skeletonMock = std::static_pointer_cast<IPCSkeletonMock>(skeleton);
    auto appManagerMock = std::make_shared<AppManagerMock>();
    ...
    EXPECT_CALL(*appManagerMock, IsSystemSA()).WillOnce(Return(true));
    EXPECT_CALL(*deviceProfileConnectorMock, GetAllAclIncludeLnnAcl()).WillOnce(Return(profiles));
    EXPECT_CALL(*deviceProfileConnectorMock, IsLnnAcl(_)).WillOnce(Return(false));
    EXPECT_CALL(*multipleUserConnectorMock, GetCurrentAccountUserID()).WillOnce(Return(100));
    EXPECT_CALL(*deviceProfileConnectorMock, CheckSinkShareType(_, _, _, _, _)).WillOnce(Return(false));
