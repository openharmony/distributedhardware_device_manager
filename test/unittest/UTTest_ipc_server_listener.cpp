 ...
 * @tc.type: FUNC
 */
HWTEST_F(IpcServerListenerTest, GetAllProcessInfo_001, testing::ext::TestSize.Level0)
{
    std::shared_ptr<IpcServerListener> ipcServerListener = std::make_shared<IpcServerListener>();
    std::vector<ProcessInfo>  processInfo = ipcServerListener->GetAllProcessInfo();
    ASSERT_NE(processInfo.empty(), true);
}
} // namespace
} // namespace DistributedHardware
} // namespace OHOS
